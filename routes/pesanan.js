  ...
  try {
    ...
    const [rows] = await db.query(`
      SELECT
        ...
        pay.metode AS bayar_metode,
        pay.nama_penyedia AS bayar_penyedia,
        pay.status_bayar,
        kr.kode_kurir
      FROM pesanan p
    ...
    res.render('pesanan/index', {
      ...
      filters: { status: status || 'semua', q: q || '' }
    });
  } catch (err) { next(err); }
});
  ...
  try {
    const [[pesanan]] = await db.query(`
      SELECT p.*, pl.nama AS pelanggan_nama, pl.email AS pelanggan_email, pl.no_hp,
             a.label, a.penerima, a.no_hp_penerima, a.alamat_lengkap, a.kota, a.provinsi, a.kode_pos
      FROM pesanan p
      JOIN pelanggan pl ON pl.id_pelanggan = p.id_pelanggan
    ...
    res.render('pesanan/detail', {
      title: `Pesanan #TK-${pesanan.id_pesanan} - TokoKita`,
      pageTitle: `Pesanan #TK-${String(pesanan.id_pesanan).padStart(4,'0')}`,
      breadcrumb: 'Home / Pesanan / Detail',
      pesanan, items, bayar, kirim
    });
  } catch (err) { next(err); }
});
...
router.post('/:id/status', async (req, res, next) => {
  try {
    ...
    await db.query('UPDATE pesanan SET status_pesanan = ? WHERE id_pesanan = ?', [status, req.params.id]);
